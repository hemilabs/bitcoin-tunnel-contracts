...
*
* When deployed, the address of the GlobalConfig that will manage this factory must be set as
* the factoryStatusAdmin so that the GlobalConfig can activate the vault factory when it 
* becomes the active factory system, and later deprecate it (and downstream vaults created
* by it) when it is replaced with a new vault factory implementation.
*/
contract SimpleBitcoinVaultFactory is IVaultFactory {
    /**
    * The onlyFactoryStatusAdmin modifier is used on all functions that should
    * *only* be callable by the factoryStatusAdminAddr (a GlobalConfig contract).
    */
    ...
    /**
    * The factoyActiveCheck modifier is used on all functions that should *only* be 
    * callable when the factory is active (activated and not yet deprecated).
    *
    * factoryActive should always be false if factoryDeprecated is true,
    * but checks both anyway.
    */
    ...
    // The minimum soft collateralization threshold that a SimpleBitcoinVaultFactory can
    // be configured to use for vaults
    uint256 public constant MIN_SOFT_COLLATERALIZATION_THRESHOLD = 130; // 130%

    // The minimum hard collateralization threshold that a SimpleBitcoinVaultFactory can
    // be configured to use for vaults
    uint256 public constant MIN_HARD_COLLATERALIZATION_THRESHOLD = 110; // 110%

    // The admin, should be set to the GlobalConfig which will activate this vault factory
    // when it becomes the allowed factory system, and later deprecate it when replaced.
    ...
    // Whether the factory is active. A factory is active after it has been activated
    // until it has been deprecated.
    bool factoryActive;
    ...
    */
    constructor(address _configAdmin,
        ...
        // Deploy a new SimpleGlobalVaultConfig with the provided pass-through values.
        // Note that the deprecationAdmin provided to this constructor is for calling the
        // deprecation function on this SimpleBitcoinVaultFactory, which in turn will call
        // the deprecation function on the vaultConfig. As a result, the SimpleGlobalVaultConfig
        // is created with the address of this SimpleBitcoinVaultFactory as the deprecation
        // admin, not the factoryStatusAdmin which will call the deprecate function on this
        // SimpleBitcoinVaultFactory.
    ...
    /**
    * Creates a new vault for custodying Bitcoin. Can only be called if this factory is active
    * (has been activated and has not yet been deprecated).
    *
    ...
    * fields.
    *
    * @param setupAdmin Not used for SimpleBitcoinVault
    ...
    * @param vaultType Not used by SimpleBitcoinVaultFactory as only one implementation
    *                  (SimpleBitcoinVault) is supported
    * @param extraInfo Not used for SimpleBitcoinVault
    *
    ...
    */
    function createVault(address setupAdmin,
        ...
        SimpleBitcoinVault vault = new SimpleBitcoinVault(tunnelAdmin, operatorAdmin, bitcoinTokenContract, vaultConfig);
    ...
    */
    function activateFactory() external onlyFactoryStatusAdmin {
        // Ensure the factory isn't already active and is not deprecated.
        // factoryActive is set to false when the factory is deprecated,
        // so both being false indicates pre-activation initial state.
        require(!factoryActive && !factoryDeprecated, "can only activate a factory that has not been activated yet");
    ...
    */
    ...
    /**
    * Marks this vault factory as deprecated when it is being replaced.
    * Makes the appropriate downstream deprecate call on the SImpleGlobalVaultConfig.
    *
    * Only callable by the factoryStatusAdmin which should be a GlobalConfig, and
    * can only be called while the factory is active.
    */
    ...
    */
    ...
    /**
    * Returns the admin of this factory which is able to deprecate vaults
    * created by this factory system.
    *
