...
interface IBitcoinVault is CommonStructs {
    ...
    */
    ...
    /**
     * Returns whether the particular vault implementation requires a preconfirmation step for a
     * deposit.
     *
    ...
    */
    ...
    /**
     * Pre-confirms a deposit, used only on vaults where requiresDepositPreconfirmation() returns
     * true. Vaults can use this if they need a two-phase deposit process, such as a user revealing
     * the script behind a deposit address the user themselves generated, and the vault operators
     * having to take an on-Bitcoin action with this information before the deposit is considered
     * finalized and BTC representative tokens should be minted.
     *
    ...
    */
     ...
     * being confirmed.
     *
     * Security Note: Implementations should only allow the tunnelAdmin (generally a
     * BitcoinTunnelManager) to call this function, as it is the contract responsible for minting
     * hBTC based on a confirmed deposit.
     *
     * @param txid The transaction ID of the deposit transaction on Bitcoin
     * @param outputIndex The index of the output in the transaction which constitutes the deposit
     * @param extraInfo Arbitrary bytes that can contain additional data necessary to confirm a deposit
    ...
    */
    function confirmDeposit(bytes32 txid, uint256 outputIndex, bytes memory extraInfo) external returns (bool success, uint256 totalDepositSats, uint256 netDepositSats, address depositor);

    /**
     * Optional - only implemented by a IBitcoinVault implementation that permits its operators to
     * mint collected fees as hBTC.
     *
     * Returns an amount of sats to mint that can be different than amountToMint to give vaults
     * flexibility, for example ignoring amountToMint if for some reason an operator should always
     * mint all available fees.
     *
     * Security Note: Implementations should only allow the tunnelAdmin (generally a
     * BitcoinTunnelManager) to call this function, as it is the contract responsible for minting
     * the corresponding hBTC.
     * 
    ...
    */
     ...
     * The actual amount of the withdrawal will be lower than amountSats to account for the
     * transaction fee, which each implementation will determine independently.
     *
     * Security Note: Implementations should only allow the tunnelAdmin (generally a
     * BitcoinTunnelManager) to call this function, as it is the contract responsible for burning
     * hBTC corresponding to the withdrawal.
     *
    ...
    */
     ...
     *
     * Security Note: Implementations should only allow the tunnelAdmin (generally a
     * BitcoinTunnelManager) to call this function, as it is the contract responsible for reminting
     * hBTC based on a successful withdrawal challenge.
     *
    ...
    */
    ...
    /**
     * Returns the lower and upper bounds of withdrawal amounts the vault will accept. Function must
     * revert if no withdrawal will be accepted.
     *
