...
/**
* Interface for a VaultFactory implementation, which allow the creation of a specific type of vault.
* Each VaultFactory implementation will have its own internal state for what configuration to use
* to setup the specific vault type.
*
* If it is desired to permit multiple different implementations of a vault custodianship system,
* a single IVaultFactory implementation can be created which allowxs deployment of different
