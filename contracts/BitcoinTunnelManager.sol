...
/**
* The BitcoinTunnelManager is deployed once for the entire Bitcoin Tunnel System
* and is the central interaction point for:
*   - Creating new vaults
*   - Preconfirming and confirming deposits
*   - Initiating withdrawals
*   - Challenging a withdrawal
*
* The BitcoinTunnelManager is not governed directly by and admin, but upon initialization
* it deploys a GlobalConfig which delegates administration of specific configuration
* options to different admins. Specifically:
...
* 
* ** hBTC Token Contract **
* The BitcoinTunnelManager deploys its own instance of the hBTC token contract, which
* will continue to be the only hBTC token representing BTC custodied with this instace
* of the Bitcoin tunnel system for the duration of its lifetime.
* 
* ** Upgradeable BTC Custodianship System **
* Over time, the custodianship mechanism for BTC used by the BitcoinTunnelManager can
* change. For example, a BitcoinTunnelManager could be initially deployed using a
* over-collateralized address-based custodianship system, and then later be updated
* to use a BitVM-style construction.
* 
* When the custodianship mechanism is updated by the appropriate admin in GlobalConfig (which
* could be a single EOA, a Safe, or a custom smart contract that is controlled via some on-chain
* governance mechanic), the active Vault Factory is updated, meaning that all new vaults
* created will use the new custodianship mechanism (Vault implementation).
*
* This custodianship upgrade will deprecate all vaults created using the old mechanism,
* which if implemented correctly in the vault implementation itself will block deposit
* confirmations to vaults of the old system after a grace period to phase them out.
* 
* ** BTC Deposit Process **
...
* differently. A specific Vault implementation can either credit the operator(s) with
* ownership of native BTC held by the vault which they could withdraw directly, or
* by recording the fees and allowing the Operator to claim them and mint hBTC via the
* BitcoinTunnelManager.
* 
* To support the 2nd model of Vault fee claiming, the BitcoinTunnelManager implements
* a "mintVaultFees" which a specific Operator can call
* 
* The BitcoinTunnelManager can optionally charge an additional fee on each deposit (above
* the one charged by the vault itself) 
*/
contract BitcoinTunnelManager {
    ...
    /**
    * The onlyVaultCaller modifier is used on all functions that should
    * *only* be callable by the vaults managed by this Bitcoin Tunnel Manager.
    */
    ...
    // The GlobalConfig which manages permissions for vault custodianship implementation
    // upgrades (IVaultFactory used),  global vault creation pausing and whitelisting,
    // and withdrawal pausing and whitelisting.
    GlobalConfig public globalConfig;

    // The bitcoinKitContract the BitcoinTunnelManager will use
    ...
    // Mapping storing the address of each created vault to lookup whether a particular
    // address refers to a vault deployment created by and managed under this Bitcoin
    // Tunnel Manager.
    mapping(address => bool) public vaultList;


    constructor(address initialAdmin, uint256 initialVaultFactoryUpgradeDelay, uint256 minimumVaultFactoryUpgradeDelayToSet, IBitcoinKit bitcoinKitAddr) {
        ...
        globalConfig = new GlobalConfig(initialAdmin, initialVaultFactoryUpgradeDelay, minimumVaultFactoryUpgradeDelayToSet);

        // Deploy a new BTCToken which is the ERC20 contract for hBTC that this BitcoinTunnelManager will mint/burn
        btcTokenContract = new BTCToken(address(this));

        bitcoinKitContract = bitcoinKitAddr;
    }

    /**
    * Called by vaults to burn hBTC that the vault itself has collected during liquidation
    * that should be burnt. Only callable by vaults to prevent user mistakes, and the vault
    * collects the hBTC first using standard ERC20 transferFrom() to prevent a vault
    * implementation from being able to burn hBTC directly as an extra layer of authority
    * separation.
    *
    ...
    */
    ...
    /**
     * Called by anyone who wants to deploy a new vault that will be used under this
     * Bitcoin Tunnel Manager. The code implementation of the vault that will be deployed
     * depends on the current IVaultFactory contract that this BitcoinTunnelManager is
     * configured to use.
     *
     * Caller specifies a vaultType which is passed through to the IVaultFactory to allow
     * users to specify one of several vault types if the IVaultFactory supports creation
     * of multiple different vaults based on different custodianship model implementations.
     *
    ...
    /**
    * Preconfirms a deposit if required by the underlying vault implementation.
    * The preconfirm deposit function can be called directly on the IBitcoinVault
    * implementation, but we provide a pass-through here so users only have to interact
    * with the BitcoinTunnelManager for all deposit needs.
    *
    ...
    */
    ...
    /**
    * Confirms a deposit transaction on Bitcoin to the corresponding vault.
    * If vault confirms that the deposit was accepted, mint the appropriate amount
    * of hBTC to the EVM address extracted from or otherwise associated with
    * the confirmed deposit that the vault returns.
    *
    * If the vault custodianship implementation requries predeposit, the txid used
    * for preconfirmation can be different than the one used for confirmation, allowing
    * implementations that preconfirm a deposit to an initial holding UTXO which is
    * then swept to the vault in a separate transaction which is used for confirmation.
    *
    ...
    */
    ...
    /**
    * Initiates a withdrawal on the specified vault. The corresponding amount of
    * hBTC that matches the withdrawal request will be burned from the sender,
    * and the vault will calculate a fee to charge and return whether the initialization
    * was successful, the fees charged, and a 32-bit UUID specific to the vault.
    *
    * This function will calculate a 64-bit UUID, comprised of the vault index in 
    * the higher 32 bits, and the vault-returned uuid in the lower 32 bits, creating
    * a UUID that can be used to track the withdrawal status and be decomposed to
    * convert it to a vault-specific UUID.
    *
    ...
    */
    function initiateWithdrawal(uint32 vaultIndex, string memory btcAddress, uint256 amount) external returns (uint256 feeSats, uint64 uuid) {
        ...
        if (globalConfig.vaultCreationWhitelistEnabled()) {
            require(globalConfig.isAddressPermittedToWithdraw(msg.sender), "withdrawal whitelisting enabled and caller is not whitelisted");
        }

        IBitcoinVault vault = vaults[vaultIndex];

        // Convert the BTC address to a script. Vaults support withdrawing to arbitrary script (within size bounds),
        // but BitcoinTunnelManager only supports addresses to protect against user error.
        bytes memory btcWithdrawalScript = bitcoinKitContract.getScriptForAddress(btcAddress);
    ...
    /**
    * Challenges a withdrawal based on the global uuid (which is vault idnex + vault specific UUID).
    *
    ...
    */
    function challengeWithdrawal(uint64 uuid, bytes memory extraInfo) external returns (bool success) {
        ...
        (bool challengeSuccess, uint256 satsToCredit, address originalWithdrawer) = vault.challengeWithdrawal(vaultSpecificUUID, extraInfo);
    ...
    *
    * Note that this function passes through msg.sender to the mintOperatorFees function on the vault, which the
    * vault trusts the BitcoinTunnelManager to correctly relay.
    *
    * Callable by anyone, but will only succeed when called by a an address that has the right to mint fees on the
    * corresponding vault. Vaults can implement the ability for multiple different operators to mint their own fees,
    * for custodianship mechanisms that support multiple operators participating in a single vault.
    *
