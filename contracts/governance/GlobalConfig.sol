...
contract GlobalConfig {
    ...
    /**
    * The onlyGlobalConfigAdmin modifier is used on all functions that should
    * *only* be callable by the globalConfigAdmin.
    */
    modifier onlyGlobalConfigAdmin() {
        require(msg.sender == globalConfigAdmin);
        _;
    }
    ...
    * be callable by either the globalConfigAdmin *or* a specific address.
    * 
    ...
    */
    ...
    // How many seconds between when an upgrade of the VaultFactory is initiated
    // and when the new VaultFactory goes into effect (meaning a new custodianship
    // mechanism begins to be used by the BitcoinTunnelManager).
    uint256 public vaultFactoryUpgradeDelay;

    // The minimum vaultFactoryUpgradeDelay that can be set
    uint256 public minimumVaultFactoryUpgradeDelay;

    // Set when the vaultFactoryUpgradeDelay is lowered, to the current time at lowering
    // plus the former vaultFactoryUpgradeDelay, to ensure that lowering a delay cannot
    // be used to perform an upgrade without users having the original expected time to
    // respond to the change in vault upgrade governance.
    // For example if delay was 7 days and is changed to 5 days, this is set to current+7 
    // days and no vault upgrade can happen before that current+7 days timestamp, even if
    // a vault factory upgrade was queued immediately after lowering the delay to 5 days.
    uint256 public grandfatheredMinVaultFactoryUpgradeTime;

    // The VaultFactory that the BitcoinTunnelManager can use to allow creation of
    // new BitcoinVaults - this defines both the permitted BitcoinVault implementation
    // as well as the ImplGlobalVaultConfig that will be used for all instances.
    IVaultFactory public vaultFactory;

    // The VaultFactory that an admin has started an upgrade to which hasn't gone into
    // effect yet
    ...
    // A mapping storing deprecated vaults. Not used in any of the Bitcoin Tunnel System
    // contracts, only provided for external contracts/services to easily check
    // previous vault factories and the status of each vault that they deployed.
    ...
    // The vaultFactoryUpgradeAdmin can upgrade the VaultFactory implementation (kicks off waiting period)
    address public vaultFactoryUpgradeAdmin;

    // The vaultFactoryUpgradeBypassAdmin can force an upgrade to occur without the required delay elapsing
    address public vaultFactoryUpgradeBypassAdmin;

    // A one-time latch that only globalConfigAdmin can set to permanently disable all future
    // vault upgrades. Only applies to future vault upgrade initiations, a vault upgrade in progress
    ...
    // The vaultCreationWhitelistModificationAdmin can add *and* remove addresses to/from the 
    // vault creation whitelist if and when it is enabled.
    ...
    // A one-time latch that only globalConfigAdmin can set to permanently disable future vault creation whitelisting
    bool public vaultWhitelistingPermDisabled = false;

    // A one-time latch that only globalConfigAdmin can set to permanently disable future vault creation pausing
    ...
    // The withdrawalPauseAdmin can pause withdrawals from vaults globally, but is unable to unpause them
    address public withdrawalPauseAdmin;

    // The withdrawalUnpauseAdmin can unpause withdrawals from vaults globally, but is unable to pause them
    address public withdrawalUnpauseAdmin;

    // the withdrawalWhitelistEnableAdmin can enable whitelisting. Enabling whitelisting will
    // deploy a new AddressWhitelist contract that has no addresses whitelisted initially.
    ...
    // The withdrawalWhitelistAdditionAdmin can *only* add addresses to the withdrawal
    // whitelist, but is unable to remove addresses from the whitelist.
    ...
    // A one-time latch that only globalConfigAdmin can set to permanently disable future withdrawal whitelisting
    bool public withdrawalWhitelistingPermDisabled = false;

    // A one-time latch that only globalConfigAdmin can set to permanently disable future withdrawal pausing
    bool public withdrawalPausingPermDisabled = false;

    /**** [END VAULT WITHDRAWAL PERMISSION VARIABLES] ****/


    // The globalConfigAdmin can update permissions for who can update all other config values,
    // and can update all other config values themselves.
    address public globalConfigAdmin;


    /**
    * Create a new GlobalConfig with an initial single admin set for all update
    * permissions, along with an initial upgrade delay for factory implementations
    * and an absolute minimum upgrade delay that can ever be set, along with
    * an initial vault factory to use.
    * 
    * The globalConfigAdmin set in the constructor can never be updated, as the
    * GlobalConfig should only ever be deployed and used by a single BitcoinTunnelManager.
    * 
    * It is permitted to create a GlobalConfig with a 0-second upgrade delay.
    *
    * The GlobalConfig is not setup with a vault factory initially, so that it can be
    * deployed during the BitcoinTunnelManager deployment. Then the address of the GlobalConfig
    * is known and can be used for setting up the initial vault factory which later gets
    * set in this GlobalConfig.
    *
    ...
    */
    constructor(address globalConfigAdminToSet, 
                ...
                uint256 minimumVaultFactoryUpgradeDelayToSet) notZeroAddress(globalConfigAdminToSet) {
        require(initialVaultFactoryUpgradeDelay >= minimumVaultFactoryUpgradeDelayToSet, "initial factory upgrade delay must be >= minimum specified delay");
    ...
    /**
    * Sets the initial vault factory after construction, callable by either the
    * globalConfigAdmin or the vaultFactoryUpgradeAdmin.
    *
    * After initial deployment of the GlobalConfig, the vaultFactoryUpgradeAdmin
    * could be changed from the default (same as globalConfigAdmin) to delegate
    * this initialization.
    *
    * The initial vault factory is set instantly as it's part of the overall
    * initialization process and there is no existing vault system that needs
    * to be phased out.
    *
    ...
    */
    ...
    /**
    * Performs the sanity checks on a vault factory before accepting it
    * as the new vault factory. This does not protect against a malicious implementation
    * of IVaultFactory, and is only a sanity check to protect against accidental
    * misconfiguration.
    *
    * Sanity Checks:
    ...
    */
    function validateVaultFactoryConfig(IVaultFactory vaultFactoryToCheck) private view {
        require(vaultFactoryToCheck.getFactoryStatusAdmin() == address(this), "vault factory does not have this global config set as status admin");
        require(vaultFactoryToCheck.getChildrenCount() == 0, "vault factory already deployed vaults");
        require(!vaultFactoryToCheck.isFactoryActive(), "vault factory is already active");
        require(!vaultFactoryToCheck.isFactoryDeprecated(), "vault factory is already deprecated");
    }
    ...
    /**
    * Permanently disable future vault upgrades, only callable by globalConfigAdmin.
    * Will only disable initializing future vault upgrades, and will permit an
    * in-progress vault upgrade to finish.
    */
    ...
    /**
    * Permanently disable bypassing future vault upgrade activation delays, only
    * callable by globalConfigAdmin.
    */
    ...
    /**
    * Permanently prevent enabling the address whitelist for vault creation, only
    * callable by globalConfigAdmin.
    */
    ...
    */
    ...
    /**
    * Permanently prevent enabling the address whitelist for withdrawal initiation,
    * only callable by globalConfigAdmin.
    */
    ...
    /**
    * Permanently prevent pausing of withdrawal initiation, only callable 
    ...
    */
    ...
    /**
    * Returns whether a vault factory upgrade is in progress, determined by whether
    * there is a non-zero pendingActivationVaultFactory set.
    * 
    ...
    */
    ...
    /**
    * Begins the upgrade process for the VaultFactory implementation, which sets pendingActivationVaultFactory
    * and sets the vaultUpgradeStartTime to the current time.
    * 
    * If there is already a pending VaultFactory upgrade, it will be replaced with this new VaultFactory
    * and the countdown to upgrade activation will be reset (meaning the previous not-yet-activated
    * VaultFactory will never become active).
    * 
    ...
    */
    function initiateVaultFactoryUpgrade(IVaultFactory newVaultFactory) external notZeroAddress(address(newVaultFactory)) senderPermissionCheck(vaultFactoryUpgradeAdmin) {
        require(vaultUpgradePermDisabled == false, "vault upgrades are permanently disabled");
    ...
    /**
    * Reject (delete) a pending vault factory upgrade. Can be called by globalConfigAdmin or
    * the vaultFactoryUpgradeAdmin.
    */
    ...
    * Assumes that the upgrade is allowed (caller must check that either the activation
    * time window has passed, or is allowed to be bypassed).
    */
    ...
    /**
    * Processes the VaultFactory upgrade after the activation time has been reached.
    ...
    */
    function finalizeVaultFactoryUpgrade(bool force) external {
        ...
        // Ensure that the minimum elapsed time based on current delay has passed, and also that
        // the current timestamp is past the grandfatheredMinVaultFactoryUpgradeTime, which is set
        // to prevent a change which lowers the upgrade delay from allowing an upgrade sooner than
        ...
        if (elapsed >= vaultFactoryUpgradeDelay && block.timestamp >= grandfatheredMinVaultFactoryUpgradeTime) {
            // Do not need to check that pendingActivationVaultFactory is set, as this is checked
            // by the finalizeVaultFactoryUpgradeInternal function we call.
            finalizeVaultFactoryUpgradeInternal();
            return;
        } 

        // Special bypass - if enough time has not elapsed but the sender is either the globalConfigAdmin
        // or the vaultUpgradeBypassAdmin and the vaultUpgradeBypassPermDisabled latch has not been set, 
        // then the activation time window can be bypassed.
        if (msg.sender == globalConfigAdmin || msg.sender == vaultFactoryUpgradeBypassAdmin) {
            if (force) {
                ...
                revert("the caller is has upgrade delay bypass permissions but did not specify force=true to bypass");
            }
        } else {
    ...
    */
    function updateVaultFactoryUpgradeDelay(uint256 newVaultFactoryUpgradeDelay) external senderPermissionCheck(vaultFactoryUpgradeAdmin) {
        require(newVaultFactoryUpgradeDelay >= minimumVaultFactoryUpgradeDelay, "New upgrade delay must be >= the set minimum upgrade delay");
    ...
    */
    ...
    /**
    * Unpauses creation of new vaults, only callable by globalConfigAdmin and vaultCreationUnpauseAdmin
    */
    ...
    /**
    * Enables the whitelist for addresses permitted to create new vaults, only callable by globalConfigAdmin
    * and vaultCreationWhitelistEnableAdmin.
    */
    ...
    /**
    * Disables the whitelist for addresses permitted to create new vaults, only callable by globalConfigAdmin
    * and vaultCreationWhitelistDisableAdmin.
    */
    ...
    /**
    * Adds an address to the whitelist of addresses permitted to create new vaults, only callable by
    * globalConfigAdmin, vaultCreationWhitelistModificationAdmin, and vaultCreationWhitelistAdditionAdmin
    *
    ...
    */
    ...
    /**
    * Removes an address from the whitelist of addresses permitted to create new vaults, only callable by
    * globalConfigAdmin or vaultCreationWhitelistModificationAdmin.
    *
    ...
    */
    ...
    /**
    * Disables the whitelist for addresses permitted to withdraw, only callable by globalConfigAdmin
    * and withdrawalWhitelistDisableAdmin.
    */
